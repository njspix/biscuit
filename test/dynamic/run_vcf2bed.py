...
def main(biscuit_dir, out_dir, vcf_dir, force):
    ...
    if compare_files.compare_files('.bed', f'../data/dynamic/{out_dir}/current', f'{out_dir}/new'):
        ...
        if logger.level == logging.DEBUG:
            logger.error(f'Mismatch in files: *.bed - see above to see differences')
        else:
            logger.error(f'Mismatch in files: *.bed - set `verbose = true` in config.toml to see differences')
        sys.exit(1)
